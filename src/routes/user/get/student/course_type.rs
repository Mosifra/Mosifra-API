...
use super::domain::GetCourseTypePayload;
...
pub async fn get_student_course_type(
	auth: AuthGuard,
) -> Result<Json<GetCourseTypePayload>, Status> {
	let generic_user = auth.get_generic_user().await?;

	if generic_user.is_student() {
		let student = generic_user.to_student()?;
		Ok(Json(GetCourseTypePayload {
			success: true,
			course_type: Some(student.get_course_type().await?),
		}))
	} else {
